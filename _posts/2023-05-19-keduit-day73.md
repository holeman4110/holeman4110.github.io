---
layout: single
title: "클라우드 컴퓨팅과 보안솔루션을 활용한 DC 엔지니어 양성교육 - Day73"
categories: keduit
tags:
  [
    2023/05/19,
...
```
- Manager(3G mem)
Nat(Addr: 192.168.0.100/24, Gateway: 192.168.0.2, DNS: 192.168.0.2 / 168.126.63.1), Hostname : manager
- Worker1 (1G mem)
Nat(Addr: 192.168.0.101/24, Gateway: 192.168.0.2, DNS: 192.168.0.2 / 168.126.63.1), Hostname : worker1
- Worker2 (1G mem)
Nat(Addr: 192.168.0.102/24, Gateway: 192.168.0.2, DNS: 192.168.0.2 / 168.126.63.1), Hostname : worker2
...
# firewall-cmd --zone=public --add-port=2377/tcp --permananet //Swarm Manager
# firewall-cmd --zone=public --add-port=2377/udp --permananet
# firewall-cmd --zone=public --add-port=7946/tcp --permananet //노드간 통신
# firewall-cmd --zone=public --add-port=7946/udp --permananet
# firewall-cmd --zone=public --add-port=4789/tcp --permananet //ingress overlay network
# firewall-cmd --zone=public --add-port=4789/udp --permananet
# firewall-cmd --reload
...
//Worker1,2
# docker swarm join --token {token number} 192.168.0.100:2377

//Manager
# docker node ls

//Optional
# docker swarm leave --force //Ready -> Down
# docker node rm worker1 //Down상태인 node 제거
# docker swarm join-token manager //manager가 되기위한 token 확인
# docker swarm join-token worker //worker가 되기위한 token 확인
# docker node promote worker1 //worker1(worker -> manager)
# docker node demote worker1 //worker1(manager -> worker)
```

```
Docker - Container
Swarm Mode - Replica, Service(Replica의 집합), Task(Service 내의 특정 데몬)
