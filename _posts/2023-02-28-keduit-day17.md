...
&nbsp;&nbsp;&nbsp;&nbsp; 위의 설정에서 SW1 기준으로 1.1.234.0 네트워크를 가기위한 정적 라우팅(Static Route)을 양쪽 interface를 통해 설정하면 자동으로 부하분산이 된다. (eg. 1-4번까지 4개의 패킷을 보낼 때, 1개씩 번갈아가면서 보냄) 그리고 이것을 tcp가 재정렬하여 목적지에서 수신하게 된다. 하지만 이러한 불필요한 부하분산은 네트워크를 효율적으로 사용할 수 없게 만든다.
...
&nbsp;&nbsp;&nbsp;&nbsp; 기존에는 모든 라인의 bandwidth가 default 값인 100,000이었으므로 한쪽을 Main으로 두고 Backup을 두는 방식으로 구성할 필요가 없었다. 하지만 위 설정에서는 SW1에서 양쪽을 다 정적 라우팅을 해주면 양쪽 경로의 bandwidth가 크게 차이남에도 불구하고 불필요하게 자동으로 부하분산을 하게된다. 따라서 좌측의 AD값을 1보다 크게 설정해주면 라우팅 테이블에서 사라졌다가 우측 라인이 죽게되면 좌측이 백업으로 된다. 이를 Floating (Static) Route라 한다.
...
&nbsp;&nbsp;&nbsp;&nbsp; 1. HSRP(Hot Standby Redundancy Protocol)는 Cisco 전용 Protocol로 UDP 1985번 포트를 사용하며 구성 방법은 아래와 같다.
...
# standby 234 preempt delay minimum 5 //5자리 secs
...
Cloud 생성하여 VMnet포트 맞춰서 SW4에 붙인후 아래 설정

![ip_setting](https://user-images.githubusercontent.com/124491456/221794036-5c53f18f-56ab-4a9f-8624-f87a95128afc.png)

![image](https://user-images.githubusercontent.com/124491456/221796303-8e9d5e1e-a3db-4e28-95fd-23c62c5c2fdf.png)

![image](https://user-images.githubusercontent.com/124491456/221796415-ea7f2110-8c90-4ed1-b4e3-48537b71db46.png)

![image](https://user-images.githubusercontent.com/124491456/221796589-46825645-3b85-4e94-9174-20aa6030a813.png)

![image](https://user-images.githubusercontent.com/124491456/221796675-e01ec24e-655e-409d-8334-4da6ed0bf0ab.png)

![image](https://user-images.githubusercontent.com/124491456/221796788-87da194f-5c43-4a9b-8df4-45edb012c44d.png)
Gateway 200아니라 100임
...
Cloud에 VMnet4번 붙이고
VMworkstation에서 VMnet4번 할당된 vp켜주고 ip설정 자동으로 받기로 변경

```
//Client 하나 새로 생성 후 VM에서 해당 vp켜준 후 ip 설정 자동으로 받기로 변경
cmd
# ipconfig /all
```
