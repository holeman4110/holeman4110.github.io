...
```
Change the logical volume capacity named v0 from 190M to 300M. and the size of the floating range should set between 280 and 320. (This logical volume has been mounted in advance.)
```

#### 2. 예제2

```
Create a volume group(lv), and set 8M as a extends. Divided a volume group containing 50 extends on volume group lv (lvshare), make it as xfs file system, and mounted automatically under /dev/sdc. And the size of the floating range should set between 380M and 400M.
```

### 2-3. Swap Partition

```
Create a 1G partition in /dev/sdd, make it as ext4 file system, mounted automatically under /mnt/data and which take effect automatically at boot-start. Use fdisk for making swapfile.
```
