...
```
# dnf install -y httpd
# semanage port -l | grep http //SELinux에 등록된 port 리스트중 http가 들어가는 포트 조회. 82번이 http용으로 추가가 안돼있음
...
// 웹브라우저 상에서 ip:82/filename 으로 잘 보이나 확인
...
# lvs
# lvextend -L +110M /dev/VG1/lvo1 //r(resize)
# vgs 
```
