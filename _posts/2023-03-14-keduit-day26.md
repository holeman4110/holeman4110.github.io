...
//위와 같은 경우 RIP를 구성했을 때, R2와 R3사이에 5.0.0.0/8라는 auto summary된 라우팅 정보를 주고받는다. 하지만 R2, R3입장에서 5.0.0.0/8보다 상세한 라우팅 정보가 있으므로 축약된 라우팅 정보를 받지 않는다. 따라서 R2의 lo100 네트워크와 R3의 lo100 네트워크의 통신이 불가능하다.
...
# no passive interface s1/0.34 //R3 입장에서는 loopback 뿐만 아니라 R4 방향으로도 RIP 소통을 할필요가 없다. 두개의 인터페이스를 passive로 만들어줘도 되지만, interface가 많은 경우 위처럼 모두 막은다음 필요한 인터페이스만 no passive로 만들어도 된다.
