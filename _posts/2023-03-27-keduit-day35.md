...
- Classless
- Auto Summary, Split Horizon
- IP port 88
- Unequal Cost Load Balancing이 가장 큰 장점
- Metric으로로 Bandwidth, Delay, Reliability, Load, MTU를 사용. 위 각각의 Metric을 Vector Metric이라고 하며, 상수값 K로 가중치(기본값은 K1=K3=1, K2=K4=K5=0)를 부여하여 Composite Metric을 산출한다.
- if K5=0 -> ((K1*BW)+(K2+BW)/(256-Load)+(K3*DLY))*256
- if K5≠0 -> ((K1*BW)+(K2+BW)/(256-Load)+(K3*DLY))*256*K5/(Reliability+K4)
```
